...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # basic parameters
        parser.add_argument('--config', type=str, default=None, help='optional path to a JSON/YAML config file')
        parser.add_argument('--dataroot', default='./datasets/301AF2HE_new', help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        parser.add_argument('--name', type=str, default='experiment_name', help='name of the experiment. It decides where to store samples and models')
        parser.add_argument('--easy_label', type=str, default='experiment_name', help='Interpretable name')
        parser.add_argument('--gpu_ids', type=str, default='auto', help="gpu ids: e.g. '0', '0,1,2', '-1' for CPU, or 'auto'")
        ...
        parser.add_argument('--tau', type=float, default=0.001, help='Entropy parameter')
        ...
        parser.add_argument('--max_dataset_size', type=int, default=100000, help='Maximum number of samples allowed per dataset. If the dataset directory contains more than max_dataset_size, only a subset is loaded.')
    ...
    def gather_options(self):
        ...
        # Load config file if provided and merge as new defaults (CLI still overrides)
        cfg_path = getattr(opt, 'config', None)
        if cfg_path:
            ...
            if not isinstance(cfg, dict):
                raise ValueError('Config must be a JSON/YAML object mapping option names to values.')
    ...
    def parse(self):
        ...
        # set gpu ids (support 'auto')
        gpu_field = (opt.gpu_ids or '').strip()
        if gpu_field.lower() in ('', 'auto'):
            gpu_field = '0' if torch.cuda.is_available() else '-1'
        str_ids = gpu_field.split(',')
        opt.gpu_ids = []
        ...
        # Enable CUDA synchronous error reporting only when verbose
        if opt.verbose:
        ...
        # Auto-detect dataroot if the provided one is missing or placeholder
        if not os.path.isdir(opt.dataroot) or opt.dataroot == './datasets/301AF2HE_new':
            candidates = []
            for root in [Path('./datasets'), Path('datasets')]:
                if root.is_dir():
                    for d in root.iterdir():
                        if d.is_dir() and ((d / 'trainA').exists() or (d / 'trainB').exists()):
                            candidates.append(d)
            if candidates:
                # Prefer a folder containing both trainA and trainB
                def score(p: Path):
                    return int((p / 'trainA').exists()) + int((p / 'trainB').exists())
                best = sorted(candidates, key=lambda p: (-score(p), p.name.lower()))[0]
                opt.dataroot = str(best)

        # Auto-name experiment if left as placeholder
        if not opt.name or opt.name == 'experiment_name':
            try:
                ds_name = Path(opt.dataroot).name
            except Exception:
                ds_name = 'dataset'
            opt.name = f"{ds_name}_{opt.model}"
